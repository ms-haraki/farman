...
アクティブペイン / 非アクティブペインのカレントディレクトリを互いに反映できる。
...
### ファイルマネージャパネル表示中
- 左: アクティブペインのフォーカス中ファイルの絶対パス (なければ空)。tooltip でも全パス表示
- 右: アクティブペイン要約
  - 選択あり: `N / M items selected (合計バイト)` (例: `3 / 128 items selected (3.6 MB)`)
  - 選択なし: `M items` (`..` は数えない)
- 更新タイミング: カーソル移動 / 選択トグル / アクティブペイン切替 / ディレクトリ遷移 / モデル更新
...
Settings ダイアログは 4 つのトップレベルタブ (`1. Behavior` / `2. Appearance` /
`3. Viewers` / `4. Keybindings`) で構成される。`Alt+1`〜`Alt+4` で直接タブ切替可能。
...
- ファイル種別ごとのカラーリング (4 状態グリッド)
  ...
  - 「Use custom colors for inactive pane」ON で非アクティブペイン用の色も別途設定可能
