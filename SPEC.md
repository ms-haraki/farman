...
- ショートカット (例: `Ctrl+I` 等) で入力欄をトグル ON/OFF。
- 入力中はファイルリストの行が「名前にその文字列を含むもの」だけに
  絞り込まれる (大文字小文字区別なし、ワイルドカード `*` `?` 任意で対応)。
- 既存の **Sort & Filter ダイアログ** や **`Shift+文字` 頭文字ジャンプ**
  とは別経路:
  - Sort & Filter は永続的なフィルタ条件 (ディレクトリに保存可能)。
  - 頭文字ジャンプは **カーソル移動だけ** (リストの絞り込みはしない)。
  - 即時フィルタは **その場限り** の表示絞り込み。フォーカスを外す or
    Esc で解除し、すべての行が再表示される。
- 表示中のフッタに「N / M items」のように絞り込み中であることを示す。
- 内部実装は `FilterSortProxy` 相当の追加プロキシモデル、もしくは
  既存モデルに「namePatterns 一時上書き」用 API を追加する。
...
- **パスワード付きアーカイブ**は v1.0 では非対応 (UI 設計が別途必要なため)。
  ロード時にパスワード要求を検出したらエラーダイアログで通知して通常 FS に戻る
...
- 表示項目:
  ...
  - サイズ
  - 各種日時 (更新 / 作成 / 最終アクセス)
  - パーミッション (`rwxr-xr-x` 形式と 8 進)
  - 所有者 / グループ
  - シンボリックリンクなら解決先パス
  - inode / hard link 数 (Unix 系のみ)
- **ディレクトリの場合は再帰的にサイズを計算する**:
  - 別スレッド (`PropertiesWorker` のような新規ワーカー) で配下を走査し、
    総バイト数 / 総ファイル数 / 総ディレクトリ数を集計しつつ
    ダイアログに途中経過を流し込む。
  - キャンセル可能。完了するまでサイズ表示は「計算中...」。
...
- コマンド: `app.open_terminal` (キーバインドはデフォルトで `T` 等を想定)
- アクティブペインの **カレントディレクトリ** を初期ディレクトリにして、
  Settings で指定したターミナルアプリを起動する。
- プラットフォーム別のデフォルト想定:
  - macOS: `/usr/bin/open -a Terminal {dir}` (= 標準 Terminal.app)
  - Windows: `cmd.exe /K cd /D {dir}` または `wt -d {dir}` (Windows Terminal)
  - Linux: `gnome-terminal --working-directory={dir}` 等
- 戻り値・標準出力は farman 側では拾わない。起動失敗時はログに warning。

### テキストエディタで開く

- コマンド: `app.open_in_editor` (キーバインドはデフォルトで `e` or `F4`
  あたりを想定)
- カーソル下のファイル / ディレクトリ (`..` 除く) を Settings で指定した
  テキストエディタで開く。
  - ファイルなら直接、ディレクトリなら「フォルダごと開く」を期待
    (VS Code 等プロジェクト系エディタ向け)。
- プラットフォーム別のデフォルト想定:
  - macOS: `/usr/bin/open -a TextEdit {file}` または `code {file}` (VS Code)
  - Windows: `notepad {file}`、`code {file}` 等
  - Linux: `xdg-open {file}`、`code {file}` 等

### ユーザー定義コマンド

ターミナル / エディタの 2 つに固定するのではなく、**任意の外部
アプリ / シェルコマンドを「ユーザー定義コマンド」として複数登録**
できるようにする。Total Commander の「ボタンバー / ユーザーコマンド」、
Double Commander の「Tools メニュー」相当。

- 各エントリは以下の項目を持つ:
  - **名前** (メニュー / 設定上の表示名)
  - **コマンド** (実行ファイル名 or 絶対パス)
  - **引数テンプレート** (`{dir}` `{file}` `{files}` `{name}` `{ext}`
    `{otherDir}` 等のプレースホルダを展開)
  - **作業ディレクトリ** (デフォルトは アクティブペインの cwd、
    任意で `{dir}` `{otherDir}` などを使える)
  - **アイコン** (任意。指定が無ければ汎用の歯車などを使う)
  - **メニュー表示** (`Tools` メニューに追加するか)
  - **ツールバー表示** (将来のツールバーに置くか)
- キーバインドは Keybindings タブで割り当てる
  (各エントリは `user.cmd.<id>` のようなコマンド ID を持ち、
  `CommandRegistry` に動的登録される)。
- プレースホルダの展開規則:
  - `{dir}`: アクティブペインのカレントディレクトリ
  - `{otherDir}`: 反対ペインのカレントディレクトリ
  - `{file}`: カーソル位置のファイル / ディレクトリの絶対パス
  - `{files}`: 選択ファイルの絶対パス群 (空白区切り、Shell-like
    クォート済み)。選択無しならカーソル位置 1 件にフォールバック。
  - `{name}`: カーソル位置の名前 (拡張子含む)
  - `{ext}`: カーソル位置の拡張子
- 上記の **「ターミナル」「テキストエディタ」もこのユーザー定義コマンド
  の一種として扱える** (組み込みの 2 つを優先表示しつつ、内部的には
  同じ仕組み)。

### Settings (General タブ → 外部アプリ グループ)

- **ターミナル** (組み込みの専用エントリ)
  - コマンド (実行ファイル名 or 絶対パス)
  - 引数テンプレート (`{dir}` がカレントディレクトリに展開される)
  - 例: コマンド `/usr/bin/open` 引数 `-a Terminal {dir}`
- **テキストエディタ** (組み込みの専用エントリ)
  - コマンド
  - 引数テンプレート (`{file}` がフォルダ/ファイルパスに展開される)
- **ユーザー定義コマンド** (一覧 + 追加 / 編集 / 削除 / 並び替え)
  - 名前 / コマンド / 引数テンプレート / 作業ディレクトリ /
    Tools メニュー表示 / アイコン
  - 各エントリの右に「テスト起動」ボタン。
- 入力欄の右に「テスト起動」ボタン。現在のアクティブペインのパスを
  渡して起動できるか確認できる。
- ターミナル / エディタが未設定 (空) の場合は、`QDesktopServices::openUrl`
  でフォールバック (`file.execute` と同じ挙動)。
- プレースホルダ `{dir}` `{file}` を含まない引数テンプレートでも、
  最後にパスを追加する形で起動する (= 寛容に解釈)。

### 注意点

- `QProcess::startDetached` で起動 (farman を閉じても外部アプリは生存)。
- 引数のクォートは `QProcess` のリスト形式で渡し、シェルインジェクションを
  避ける (テンプレート内で `;` `&&` などは展開しない)。
  - **どうしてもシェル機能が必要な場合**: ユーザーが明示的に
    `sh -c "..."` を組み立てて登録する形 (farman 側で勝手にシェルを
    挟まない) で吸収する。
- 設定値はそのまま `settings.json` に格納 (プラットフォーム別ではなく
  単一値)。マシン間で持ち越したいときはユーザー側で書き換える運用。
- ユーザー定義コマンドは「キーバインド + 配色」と同様に
  **インポート / エクスポート / プリセット** の対象にする
  (前述「プリセット / インポート・エクスポート」節参照)。
  プラットフォームを跨ぐと実行ファイルパスが変わるので、
  プリセットは「定義テンプレート (ラベル + プレースホルダ規約)」
  として配布し、コマンドパスは個別環境で書き換える運用。
...
- **二重起動を禁止する** (`singleInstance`、デフォルト ON) *（未実装）*
  - すでに farman が起動している状態で 2 つ目を起動しようとしたら、
    新規プロセスは起動せず既存ウィンドウを前面に出す (Linux/Windows での
    `QSharedMemory` + `QLocalServer` パターン、もしくは macOS の Cocoa
    側の active activation)。
  - OFF にすると複数インスタンスを並列に立ち上げられるようにする
    (左右別の作業を完全に分けたい等のニッチ用途)。
  - 既存インスタンスへ「フォルダを開く」要求を送れる仕組みも併せて検討
    (例: コマンドラインで渡されたパスを既存ウィンドウに開かせる)。
