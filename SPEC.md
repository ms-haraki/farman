...
CI (`.github/workflows/build.yml`) で 3 OS のアーティファクトは作って
いるが、現状は単純なバイナリ (`.app` をそのまま zip / `farman.exe` を
zip / AppImage 単体)。インストーラフォーマットへの整形と署名は未実装。
...
- ~~**CONTRIBUTING.md**~~ — 作成済み ([CONTRIBUTING.md](CONTRIBUTING.md))。
  ビルド手順 / ブランチモデル / PR 作成 / コードスタイル / 翻訳手順を整理。
