...
- **macOS** — `.dmg` (Disk Image)
  - DMG を開くと左に `farman.app`、右にアプリケーションフォルダへの
    シンボリックリンクが並ぶ「典型的な Mac インストーラ」レイアウト。
...
- **macOS**: `macdeployqt` で Qt frameworks を `.app` に埋め込み →
  Developer ID 署名 (Secrets が揃っているとき) または ad-hoc 署名 → `hdiutil`
  で UDZO 形式の `.dmg` 生成 → `notarytool` で公証 + `stapler` で staple。
  **DMG レイアウト改善 (`/Applications` シンボリックリンク + アイコン配置)**
  は未対応 — 現状はマウントすると `farman.app` が 1 個置いてあるだけで、
  ユーザーは手動で Applications にドラッグする必要がある。
