...
//   - ファイル出力 ON のときは指定ディレクトリ下の `farman-YYYY-MM-DD.log` に
//     追記する。日が変わると自動で次のファイルへ。
...
public:
  ...
  // ファイル出力 ON/OFF と保存ディレクトリ、保持日数を設定。Settings 反映時に呼ぶ。
  // retentionDays は 0 で永久保持、それ以外は日数で古いファイルを削除。
  void setFileOutput(bool enabled, const QString& directory, int retentionDays);
...
private:
  ...
  // 指定日のファイルパスを組み立てる。例: 2026-04-27 -> "<dir>/farman-2026-04-27.log"
